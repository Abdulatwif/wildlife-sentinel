    ...
    if ($action === 'send_message') {
        ...
        if (empty($content)) {
            ...
            if ($stmt->execute([$user['id'], $recipientId, $incidentId, $subject, $content])) {
                ...
                if ($recipientId) {
                    createNotification($recipientId, 'new_message', '💬 New Message from Scout',
...
$incidentsList = $pdo->prepare("
    SELECT id, category, status, description, location_lat, location_lng
    FROM incidents WHERE reporter_id = ? AND status NOT IN ('resolved', 'closed')
...
<head>
    ...
    <style>
        ...
        .zone-info { background: #f8f9fa; padding: 8px 14px; border-radius: 8px; font-size: 12px; color: #6c757d; margin-bottom: 12px; border-left: 3px solid #17a2b8; }
        ...
        .location-link { color: #17a2b8; text-decoration: none; font-size: 11px; }
            ...
            <div class="content">
                ...
                <div class="messages-container">
                    <div class="compose-section">
                        ...
                        <div class="zone-info">
                            📍 Messaging zone: <strong><?= getZoneName($user['zone_id']) ?></strong>
                            <br><span style="font-size:11px;">✅ You can message rangers and supervisors in your zone</span>
                        </div>
                        <form method="POST">
                        ...
                        <?php if (count($messages) > 0): ?>
                            ...
                            <div class="message-item <?= !$msg['is_read'] && $msg['recipient_id'] == $user['id'] ? 'unread' : '' ?> <?= $msg['parent_message_id'] ? 'is-reply' : '' ?>">
                                ...
                                <div style="font-size:11px;color:#6c757d;margin-top:4px;">
                                    📌 Related: #<?= $msg['incident_id'] ?> - <?= ucfirst($msg['incident_category']) ?>
                                    <?php if ($msg['location_lat']): ?>
                                    • <a href="https://www.google.com/maps?q=<?= $msg['location_lat'] ?>,<?= $msg['location_lng'] ?>" target="_blank" class="location-link">View Location</a>
                                    <?php endif; ?>
                                </div>
                                <?php endif; ?>
