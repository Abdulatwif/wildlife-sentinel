...
// - IMMEDIATELY notifies all rangers + supervisors in the zone
//   (in-app) — respects notify_on_incident setting
// - SILENTLY sends SMS to rangers + supervisors in the zone
//   (the scout is NEVER told about the SMS) — respects sms_enabled
...
// ============================================================
if (!function_exists('ws_scout_setting')) {
    function ws_scout_setting(string $key, $default = null) {
...
$setSmsEnabled     = (string) ws_scout_setting('sms_enabled', '1')        === '1';
$setNotifyIncident = (string) ws_scout_setting('notify_on_incident', '1') === '1';

// ============================================================
// NOTIFICATION GATE
// ============================================================
if (!function_exists('ws_scout_notify')) {
    /**
     * Wraps createNotification() only when notify_on_incident is on.
     * Returns true on success, false if suppressed or failed.
     */
    function ws_scout_notify(int $userId, string $type, string $title, string $body, ?int $incidentId = null): bool {
        if ((string) ws_scout_setting('notify_on_incident', '1') !== '1') {
            return false;
        }
        ...
        try {
            ...
            error_log('[WS-SCOUT] notify failed: ' . $e->getMessage());
            return false;
        }
    }
}
...
// ============================================================
// SILENT SMS DISPATCH (scout is NEVER told)
// ------------------------------------------------------------
// Behaviour:
//   - Respects sms_enabled global setting
//   - Uses sendSMS() if available (bool OR array return both handled)
//   - Always writes attempts to sms_logs (sent OR pending)
//   - Never returns anything user-facing
//   - All errors are logged, never surfaced
// ============================================================
if (!function_exists('silentSMSDispatchToZone')) {
    function silentSMSDispatchToZone(
        ...
        string $reporterName,
        string $zoneName,
        string $reporterRole = 'Scout'
    ): array {
        $result = ['sent' => 0, 'failed' => 0, 'skipped' => 0, 'suppressed' => 0, 'recipients' => []];

        // Global gate
        if ((string) ws_scout_setting('sms_enabled', '1') !== '1') {
            $result['suppressed'] = 1;
            return $result;
        }
        ...
        foreach ($recipients as $r) {
            ...
            if ($r['role'] === 'zone_supervisor') {
                $msg = "WS OVERSEER: {$sevUpper} {$catLabel} reported in {$zoneName} by {$reporterRole} {$reporterName}. "
                     . "Incident #{$incidentId}. Check the dashboard.";
            } else {
                $msg = "WS ALERT: {$sevUpper} {$catLabel} reported in {$zoneName} by {$reporterRole} {$reporterName}. "
                     . "Incident #{$incidentId}. Open the app NOW.";
            }

            $sent = false;

            // Try the SMS gateway if available
            if (function_exists('sendSMS')) {
                try {
                    $raw = sendSMS($phone, $msg);
                    $sent = is_array($raw) ? !empty($raw['success']) : (bool) $raw;
                } catch (Throwable $e) {
                    error_log('[WS-SCOUT-SMS] sendSMS failed for ' . $phone . ': ' . $e->getMessage());
                    $sent = false;
                }
            }
            ...
            try {
                ...
                error_log('[WS-SCOUT-SMS] sms_logs insert failed: ' . $e->getMessage());
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit_report') {
    ...
    if (empty($errors)) {
        try {
            ...
            // ------------------------------------------------
            $reporterLabel = 'Community Scout: ' . $user['full_name'];
            $notifyTitle   = '🚨 NEW Incident (Scout)';
            ...
            foreach ($recipientsNotif as $r) {
                if (ws_scout_notify((int)$r['id'], 'new_incident', $notifyTitle, $notifyBody, $newIncidentId)) {
                    $notifiedCount++;
                } else {
                    $suppressedCount++;
                }
            ...
            // ------------------------------------------------
            // 2. SILENT SMS DISPATCH — never surfaced to scout
            // ------------------------------------------------
            $zoneName  = $zone['name'] ?? ('Zone #' . $activeZoneId);
            $smsResult = silentSMSDispatchToZone(
                $pdo,
                $activeZoneId,
                $newIncidentId,
                $severity,
            ...
            // Audit — admin-only visibility (includes SMS stats)
            ...
            // ------------------------------------------------
            // 4. SCOUT-FACING MESSAGE — intentionally says NOTHING about SMS
            // ------------------------------------------------
            $message = "✅ Report submitted! Incident <strong>#{$newIncidentId}</strong> has been received.";
            if ($notifiedCount > 0) {
                $message .= "<br>🔔 The wildlife team for your area has been notified.";
            } elseif (!$setNotifyIncident) {
                // Do not alarm the scout — but do not lie either.
                $message .= "<br>Your report is recorded and will be reviewed by the team.";
            } else {
                $message .= "<br>Your report is recorded and will be reviewed by the team.";
            }
            $messageType = 'success';
            ...
            error_log('[WS-SCOUT] insert failed: ' . $e->getMessage());
            $message = 'We could not save your report. Please try again, or contact your zone ranger directly.';
            $messageType = 'danger';
        }
    } else {
...
<head>
    ...
    <title>Report Incident - Scout - Wildlife Sentinel</title>
    ...
    <style>
        ...
        .category-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)); gap: 10px; }
        .category-btn {
            padding: 14px 10px;
        ...
        .photo-previews { display: grid; grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap: 8px; margin-top: 12px; }
        .photo-preview {
        ...
        .sending-overlay .text { font-size: 16px; font-weight: 600; }
...
<body>
        ...
        <main class="main-content">
            ...
            <div class="content">
                <div class="dashboard-greeting">
                    ...
                    <p>
                        Your report will be sent <strong>immediately</strong> to the
                        wildlife team responsible for
                ...
                <form method="POST" enctype="multipart/form-data" id="reportForm" onsubmit="return prepareSubmit(event)">
                    ...
                    <div class="section">
                        ...
                        <div class="form-group">
                            ...
                            <span class="hint">Be specific — the wildlife team will use this to prepare.</span>
                        </div>
                    </div>
                    ...
                    <div class="submit-panel">
                        <div style="font-size:13px;color:#6c757d;">
                            <strong>ℹ️</strong> Your report is submitted securely to the wildlife team for your area.
                        </div>
    ...
    <div class="sending-overlay" id="sendingOverlay">
        ...
        <div class="text">Submitting report…</div>
    ...
    <script>
        ...
        // Auto-detect only after first user interaction (avoid surprise permission prompt)
