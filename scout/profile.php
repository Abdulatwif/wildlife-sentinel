...
// ============================================================
// scout/profile.php
// Community Scout — Personal Account Settings
// ------------------------------------------------------------
// Sections:
//   - Profile Information
//   - Change Password
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'update_preferences') {
        ...
        $onInc   = isset($_POST['notify_incident_updates']) ? 1 : 0;
        $onRang  = isset($_POST['notify_ranger_responses']) ? 1 : 0;
        $onNear  = isset($_POST['notify_nearby_incidents']) ? 1 : 0;
            ...
            $pdo->prepare("
                INSERT INTO user_preferences
                    (user_id, theme, language, timezone, items_per_page,
                     ...
                     notify_incident_updates, notify_ranger_responses, notify_nearby_incidents,
                ...
                ON CONFLICT (theme) DO UPDATE SET theme = EXCLUDED.theme,
                    ...
                    notify_nearby_incidents = VALUES(notify_nearby_incidents),
...
$prefs = [
    ...
    'notify_incident_updates' => 1, 'notify_ranger_responses' => 1,
    'notify_nearby_incidents' => 1,
...
try {
    $stmt = $pdo->prepare("SELECT COUNT(*) as c FROM incidents WHERE reporter_id = ?");
    $stmt->execute([$user['id']]);
    $myReportsCount = (int)$stmt->fetch()['c'];

    $stmt = $pdo->prepare("SELECT COUNT(*) as c FROM incidents WHERE reporter_id = ? AND status NOT IN ('resolved','closed')");
    $stmt->execute([$user['id']]);
    $myReportsActive = (int)$stmt->fetch()['c'];

    $stmt = $pdo->prepare("SELECT COUNT(*) as c FROM incidents WHERE reporter_id = ? AND status = 'resolved'");
    $stmt->execute([$user['id']]);
    $myReportsResolved = (int)$stmt->fetch()['c'];
} catch (PDOException $e) {}
...
<head>
    ...
    <title>My Profile - Scout - Wildlife Sentinel</title>
    ...
    <style>
        ...
        .stat-card .info .number { font-size: 20px; font-weight: 700; color: #0d3b22; }
        ...
        <main class="main-content">
            ...
            <div class="content">
                <div class="dashboard-greeting">
                    <h1>👤 My Profile</h1>
                    <p>Manage your profile, password, and personal preferences.</p>
                </div>
                ...
                <div class="stats-grid">
                    <div class="stat-card"><div class="icon blue">📋</div><div class="info"><div class="number"><?= $myReportsCount ?></div><div class="label">Total Reports</div></div></div>
                    <div class="stat-card"><div class="icon orange">⏳</div><div class="info"><div class="number"><?= $myReportsActive ?></div><div class="label">Active Reports</div></div></div>
                    <div class="stat-card"><div class="icon green">✅</div><div class="info"><div class="number"><?= $myReportsResolved ?></div><div class="label">Resolved</div></div></div>
                    <div class="stat-card"><div class="icon purple">🏛️</div><div class="info"><div class="number" style="font-size:13px;"><?= htmlspecialchars($profileStats['zone_name']) ?></div><div class="label">Assigned Zone</div></div></div>
                </div>
                        ...
                        <div class="section">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group full">
                                    ...
                                    <span class="hint">Used for SMS alerts about your reports.</span>
                                </div>
                    ...
                    <div>
                            ...
                            <form method="POST">
                                ...
                                <div class="toggle-row">
                                    <div class="toggle-info">
                                        <div class="toggle-label">Incident Updates</div>
                                        <div class="toggle-desc">When your reports change status.</div>
                                    </div>
                                    <label class="switch">
                                        <input type="checkbox" name="notify_incident_updates" value="1" <?= (int)$prefs['notify_incident_updates'] === 1 ? 'checked' : '' ?>>
                                        <span class="slider"></span>
                                    </label>
                                </div>
                                <div class="toggle-row">
                                    <div class="toggle-info">
                                        <div class="toggle-label">Ranger Responses</div>
                                        <div class="toggle-desc">When a ranger responds to your report.</div>
                                    </div>
                                    <label class="switch">
                                        <input type="checkbox" name="notify_ranger_responses" value="1" <?= (int)$prefs['notify_ranger_responses'] === 1 ? 'checked' : '' ?>>
                                        <span class="slider"></span>
                                    </label>
                                </div>
                                <div class="toggle-row">
                                    <div class="toggle-info">
                                        <div class="toggle-label">Nearby Incidents</div>
                                        <div class="toggle-desc">Alert me when incidents are reported near my area.</div>
                                    </div>
                                    <label class="switch">
                                        <input type="checkbox" name="notify_nearby_incidents" value="1" <?= (int)$prefs['notify_nearby_incidents'] === 1 ? 'checked' : '' ?>>
                                        <span class="slider"></span>
                                    </label>
                                </div>
                        ...
                        <div class="section">
                            ...
                            <div style="margin-top:14px;">
                                <a href="../logout.php?role=scout" class="btn btn-danger" style="width:100%;justify-content:center;"
                        ...
                        <div class="section" style="border-left:4px solid #cce5ff;background:#f8fbff;">
                            <div style="font-size:13px;color:#495057;line-height:1.7;">
                                <strong>💡 Tips for Community Scouts</strong><br>
                                • Enable SMS so rangers can reach you quickly about your report.<br>
                                • Configure quiet hours if you don't want late-night alerts.<br>
                                • Keep your phone number up to date.<br>
                                • Report incidents immediately — every second matters in conservation.
                            </div>
                        </div>
                    </div>
                </div>
