            ...
            try {
                ...
                if ($stmt->execute([$user['id'], $recipientId, $incidentId, $messageType, $subject, $content, $isBroadcast])) {
                    ...
                    if ($isBroadcast && $user['zone_id']) {
                        $users = $pdo->prepare("SELECT id FROM users WHERE zone_id = ? AND role IN ('ranger', 'zone_supervisor') AND is_active = 1 AND id != ?");
                        $users->execute([$user['zone_id'], $user['id']]);
                        while ($u = $users->fetch()) {
                            createNotification($u['id'], 'new_message', '📢 Broadcast',
...
$usersList = $pdo->prepare("
    ...
    FROM users WHERE id != ? AND is_active = 1 AND role IN ('ranger', 'zone_supervisor', 'admin')
    ORDER BY full_name
");
...
<head>
    ...
    <title>Messages - Ranger - Wildlife Sentinel</title>
    ...
    <style>
        ...
        .message-item.unread { border-left-color: #007bff; background: #f0f7ff; }
        ...
        .message-type-badge { font-size: 10px; padding: 2px 10px; border-radius: 12px; font-weight: 600; }
                            ...
                            <div class="message-item <?= !$msg['is_read'] && $msg['recipient_id'] == $user['id'] ? 'unread' : '' ?> <?= $msg['parent_message_id'] ? 'is-reply' : '' ?>">
                                <div class="message-header">
                                    <div class="sender">
                                        ...
                                        <?php if ($msg['reply_count'] > 0): ?><span style="background:#6f42c1;color:white;font-size:10px;padding:1px 8px;border-radius:10px;"><?= $msg['reply_count'] ?> replies</span><?php endif; ?>
                                    </div>
                                    <div>
