<?php
// ============================================================
// tourism/my-reports.php
// Tourism / Lodge Operator — My Reports with Responder Details
// ------------------------------------------------------------
// - List all incidents the tourism operator has reported
// - Show the actual responder (name + role + phone)
...
if (!function_exists('hasRole') || !hasRole('tourism')) {
...
// GLOBAL SETTINGS
// ============================================================
if (!function_exists('ws_tour_my_setting')) {
    function ws_tour_my_setting(string $key, $default = null) {
...
$itemsPerPage = (int) ws_tour_my_setting('items_per_page', 25);
if ($itemsPerPage < 5 || $itemsPerPage > 100) $itemsPerPage = 25;

// ============================================================
// HELPERS (guarded)
// ============================================================
if (!function_exists('ws_tmy_sev_badge')) {
    function ws_tmy_sev_badge(?string $sev): string {
...
if (!function_exists('ws_tmy_status_badge')) {
    function ws_tmy_status_badge(?string $status): string {
...
if (!function_exists('ws_tmy_cat_icon')) {
    function ws_tmy_cat_icon(?string $cat): string {
...
if (!function_exists('ws_tmy_role_label')) {
    function ws_tmy_role_label(?string $role): string {
        $map = [
            'ranger'          => '🛡️ Ranger',
...
if (!function_exists('ws_tmy_safe_fetch_all')) {
    function ws_tmy_safe_fetch_all(PDO $pdo, string $sql, array $params = []): array {
        try {
...
$sql = "
    ...
    WHERE i.reporter_id = ? AND i.reporter_type = 'tourism'
";
$params = [$user['id']];
...
try {
    ...
    error_log('[WS-TOUR-MY] fetch: ' . $e->getMessage());
}
...
// ============================================================
// FETCH RESPONSE TIMELINE FOR EACH REPORT (batched, no N+1)
// ============================================================
$responsesByIncident = [];
if (count($reports) > 0) {
    ...
    $rows = ws_tmy_safe_fetch_all($pdo, "
...
try {
    $stmt = $pdo->prepare("
        ...
        FROM incidents WHERE reporter_id = ? AND reporter_type = 'tourism'
    ");
    ...
    error_log('[WS-TOUR-MY] stats: ' . $e->getMessage());
}
...
<head>
    ...
    <style>
        ...
        .report-item .report-timeline .step.done { background: #28a745; color: white; }
...
<body>
                    ...
                    <?php foreach ($reports as $report): ?>
                        ...
                        <div class="report-item status-<?= htmlspecialchars($status) ?>">
                            <div class="report-header">
                                <div>
                                    <h4>
                                        <?= ws_tmy_cat_icon($report['category'] ?? null) ?>
                                ...
                                <?= ws_tmy_sev_badge($report['severity'] ?? null) ?>
                            </div>
                            ...
                            <!-- Responder panel — the key addition -->
                            <?php if ($hasResponder): ?>
                                <div class="responder-panel">
                                    ...
                                    <div class="responder-info">
                                        <div class="name">
                                            ...
                                            <span class="role"><?= ws_tmy_role_label($report['responder_role'] ?? null) ?></span>
                                        </div>
                                        <div class="meta">
    ...
    <script>
        console.log('✅ Tourism My Reports page loaded');
