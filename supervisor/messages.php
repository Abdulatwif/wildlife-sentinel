...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'send_message') {
        ...
        if (empty($content)) {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO messages (sender_id, recipient_id, incident_id, message_type, subject, content, is_broadcast, created_at)
            ...
            if ($stmt->execute([$user['id'], $recipientId, $incidentId, $messageType, $subject, $content, $isBroadcast])) {
                ...
                if ($recipientId) {
                    createNotification($recipientId, 'new_message', '💬 New Message from Supervisor',
                        "Message from {$user['full_name']}: " . substr($content, 0, 50), $incidentId, $messageId);
                }
                
                if ($isBroadcast && $user['zone_id']) {
                    $rangers = $pdo->prepare("SELECT id FROM users WHERE zone_id = ? AND role = 'ranger' AND is_active = 1 AND id != ?");
                    $rangers->execute([$user['zone_id'], $user['id']]);
                    while ($r = $rangers->fetch()) {
                        createNotification($r['id'], 'new_message', '📢 Broadcast',
                            "Broadcast from {$user['full_name']}: " . substr($content, 0, 50), $incidentId, $messageId);
                    }
                }
...
$usersList = $pdo->prepare("
    SELECT id, full_name, role, zone_id FROM users 
...
$incidentsList = $pdo->prepare("
    SELECT id, category, status FROM incidents WHERE zone_id = ? AND status NOT IN ('resolved', 'closed')
    ORDER BY reported_at DESC LIMIT 20
");
$incidentsList->execute([$user['zone_id']]);
...
<head>
    ...
    <title>Messages - Supervisor - Wildlife Sentinel</title>
    ...
    <style>
        ...
        .message-item.unread { border-left-color: #6f42c1; background: #f8f5ff; }
        .message-item.is-reply { margin-left: 30px; border-left: 4px solid #6f42c1; background: #f0f0ff; }
        ...
        .broadcast-option { margin-top: 8px; }
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div class="broadcast-option">
                                    <label><input type="checkbox" name="is_broadcast" value="1"> 📢 Broadcast to all rangers in zone</label>
                                </div>
                            </div>
                            ...
                            <button type="submit" class="btn btn-primary btn-block" style="background:#6f42c1;">Send</button>
                        </form>
                    </div>
                            ...
                            <div class="message-item <?= !$msg['is_read'] && $msg['recipient_id'] == $user['id'] ? 'unread' : '' ?> <?= $msg['parent_message_id'] ? 'is-reply' : '' ?>">
                                <div class="message-header">
                                    <div class="sender">
                                        ...
                                        <?php if ($msg['is_broadcast']): ?><span style="color:#6f42c1;font-size:11px;">📢 Broadcast</span><?php endif; ?>
                                        <?php if ($msg['parent_message_id']): ?><span style="color:#6f42c1;font-size:11px;">↩️ Reply</span><?php endif; ?>
                                        <?php if ($msg['reply_count'] > 0): ?><span style="background:#6f42c1;color:white;font-size:10px;padding:1px 8px;border-radius:10px;"><?= $msg['reply_count'] ?></span><?php endif; ?>
                                    </div>
                                ...
                                <div style="font-size:11px;color:#6c757d;margin-top:4px;">📌 Related: #<?= $msg['incident_id'] ?> - <?= ucfirst($msg['incident_category']) ?></div>
                                <?php endif; ?>
                                
                                <div class="message-actions">
                                    <?php if ($msg['sender_id'] != $user['id']): ?>
                                    <button class="btn-small btn-primary" onclick="toggleReply(<?= $msg['id'] ?>)" style="background:#6f42c1;">↩️ Reply</button>
                                    <?php endif; ?>
                                ...
                                <div class="reply-form" id="reply-<?= $msg['id'] ?>">
                                    <form method="POST">
                                        ...
                                        <div style="display:flex;gap:8px;margin-top:8px;">
                                            <button type="submit" class="btn btn-primary btn-small" style="background:#6f42c1;">Send Reply</button>
